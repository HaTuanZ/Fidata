...
class PublicController extends Controller
{
    ...
    {
        return redirect()->route('public.account.activity', ['slug' => 'referral']);
    ...
    {
        SeoHelper::setTitle(__('Account Referrer'));

        $user = auth('member')->user();

        $affiliate_id = $user->affiliate_id;
        $members = Member::where('referred_by', $affiliate_id)->get();

        Assets::addScriptsDirectly(['vendor/core/plugins/member/libraries/cropper.js']);

        return view('plugins/member::referrer.index', compact('user', 'members'));
    }
}
